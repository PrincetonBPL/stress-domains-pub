...
* stepdown reg (time_auc time_patient time_indiff time_exponential time_geometric) treat $yfillmiss if exp_tsst, iter($iterations)	
* mat MAT_exp_tsst = r(p)
* matlist MAT_exp_tsst

* stepdown reg (time_auc time_patient time_indiff time_exponential time_geometric) treat $yfillmiss if exp_cpr, iter($iterations)	
* mat MAT_exp_cpr = r(p)
* matlist MAT_exp_cpr

* stepdown reg (time_auc time_patient time_indiff time_exponential time_geometric) treat $yfillmiss if exp_cpt, iter($iterations)	
* mat MAT_exp_cpt = r(p)
* matlist MAT_exp_cpt
...
loc prehead "\begin{table}[htbp]\centering \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} \caption{Domain-specific treatment effects -- Temporal discounting} \label{tab:REG-comparetime} \maxsizebox*{\textwidth}{\textheight}{ \begin{threeparttable} \begin{tabular}{l*{`columns'}{c}} \toprule"
...
esttab col* using "$tab_dir/REG-comparetimeall.tex", booktabs cells(none) nonum mgroups("TSST-G" "CPT" "CENT" "Overall", pattern(1 0 1 0 1 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Effect" "N" "Effect" "N" "Effect" "N" "Control mean") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress wrap replace
esttab col* using "$tab_dir/REG-comparetimeall-frag.tex", booktabs cells(none) nonum mgroups("TSST-G" "CPT" "CENT" "Overall", pattern(1 0 1 0 1 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Effect" "N" "Effect" "N" "Effect" "N" "Control mean") stats(`statnames', labels(`varlabels')) compress wrap replace
