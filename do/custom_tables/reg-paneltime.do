...
loc prehead "\begin{table}[htbp]\centering \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} \caption{Treatment effects on temporal discounting by time horizon} \label{tab:REG-horizontime} \maxsizebox*{\textwidth}{\textheight}{ \begin{threeparttable} \begin{tabular}{l*{`columns'}{c}} \toprule"
...
esttab col* using "$tab_dir/REG-horizontime.tex", booktabs cells(none) nonum mtitle("\specialcell{0 mo.\\-- 3 mo.}" "\specialcell{0 mo.\\-- 6 mo.}" "\specialcell{0 mo.\\-- 12 mo.}" "\specialcell{6 mo.\\-- 12 mo.}" "N" "\specialcell{Joint\\\(p\)-value}") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress wrap replace
