...
foreach yvar in $regvars {
	...
	psmatch2 got_insured if ~uct, pscore(pscore_insured) outcome(`yvar'_1) neighbor(1)
	loc att = r(att)
	loc seatt = r(seatt)
	...
	psmatch2 got_uct if ~insured, pscore(pscore_uct) outcome(`yvar'_1) neighbor(1)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_insured if ~control, pscore(pscore_diff) outcome(`yvar'_1) neighbor(1)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_insured if ~uct, pscore(pscore_insured) outcome(`yvar'_1) neighbor(5)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_uct if ~insured, pscore(pscore_uct) outcome(`yvar'_1) neighbor(5)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_insured if ~control, pscore(pscore_diff) outcome(`yvar'_1) neighbor(5)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_insured if ~uct, pscore(pscore_insured) outcome(`yvar'_1) neighbor(10)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_uct if ~insured, pscore(pscore_uct) outcome(`yvar'_1) neighbor(10)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
	...
	psmatch2 got_insured if ~control, pscore(pscore_diff) outcome(`yvar'_1) neighbor(10)
	loc att = r(att)
	loc seatt = r(seatt)
	pstar, b(`att') se(`seatt') prec(2)
...
loc footnote "\emph{Notes:} This table reports average treatment effects on the treated using nearest neighbor matching. Columns 1 - 3 matches using the closest neighbor. Columns 4 - 6 matches using the 5 nearest neighbors. Columns 7 - 9 matches using the 10 nearest neighbors. Standard errors are in parentheses."

esttab col* using "$output_dir/Tables/reg/$regpath.tex", booktabs cells(none) title($regtitle) mgroups("Neighbors = 1" "Neighbors = 5" "Neighbors = 10" "Sample", pattern(1 0 0 1 0 0 1 0 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Insurance" "UCT" "Difference" "Insurance" "UCT" "Difference" "Insurance" "UCT" "Difference" "\specialcell{Control Mean\\(SD)}") stats(`statnames', labels(`varlabels')) note("`footnote'") postfoot("\hline \multicolumn{@span}{p{\linewidth}}{\footnotesize @note}\\ \end{tabular}\\ \end{table}") compress replace
