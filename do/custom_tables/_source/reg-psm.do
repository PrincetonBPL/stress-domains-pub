...
** Desc: Outputs regression table comparing propensity matching algorithms
** Output: reg-psm.tex
...
loc footnote "\emph{Notes:} Columns 1 - 3 report the average treatment effect on the treated using matching to the 5 nearest neghbors. Columns 4 - 6 report results using radius matching with a 0.05 caliper. Columns 7 - 9 report results using kernel matching with 0.6 bandwidth and the Epanechnikov kernel. Standard errors are in parentheses."

esttab col* using "$output_dir/Tables/reg/$regpath.tex", booktabs cells(none) title($regtitle) mgroups("Nearest neighbor" "Radius" "Kernel" "Sample", pattern(1 0 0 1 0 0 1 0 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Insurance" "UCT" "Difference" "Insurance" "UCT" "Difference" "Insurance" "UCT" "Difference" "\specialcell{Control Mean\\(SD)}") stats(`statnames', labels(`varlabels')) note("`footnote'") postfoot("\hline \multicolumn{@span}{p{\linewidth}}{\footnotesize @note}\\ \end{tabular}\\ \end{table}") compress replace
