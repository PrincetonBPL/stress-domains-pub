** Title: sum-treatex.do
** Author: Justin Abraham
** Desc: Outputs baseline summary statistics conditional on treatment in long portrait format excluding those without ID
** Input: UMIP Master.dta
** Output: sum-treatex.do
...
foreach yvar in $sumvars {
	...
	sum `yvar'_0 if control & havenationalid_0
	loc N = `r(N)'
	...
	sum `yvar'_0 if insured & havenationalid_0
	loc N = `r(N)'
	pstar, b(`r(mean)') se(`r(sd)') pstar prec(2)
	...
	sum `yvar'_0 if uct & havenationalid_0
	loc N = `r(N)'
	pstar, b(`r(mean)') se(`r(sd)') pstar prec(2)
	...
	ttest `yvar'_0 if ~uct & havenationalid_0, by(insured)
	...
	ttest `yvar'_0 if ~insured & havenationalid_0, by(uct)
	...
	ttest `yvar'_0 if treat & havenationalid_0, by(insured)
