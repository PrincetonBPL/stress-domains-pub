...
foreach yvar in time_patient time_indiff time_exponential time_geometric time_auc time_decrimp time_stationarity {
	...
	foreach exp in exp_tsst exp_cpt exp_cpr {
		...
		if _rc == 0 {
			...
			if strpos("`yvar'", "time_geometric") loc roundN = round(`N' / 5, 1)
...
esttab col* using "$tab_dir/REG-adjustedtimeall.tex", booktabs cells(none) nonum mgroups("TSST-G" "CPT" "CENT" "Overall", pattern(1 0 1 0 1 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Effect" "N" "Effect" "N" "Effect" "N" "Control mean") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress wrap replace
esttab col* using "$tab_dir/REG-adjustedtimeall-frag.tex", booktabs cells(none) nonum mgroups("TSST-G" "CPT" "CENT" "Overall", pattern(1 0 1 0 1 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Effect" "N" "Effect" "N" "Effect" "N" "Control mean") stats(`statnames', labels(`varlabels')) compress wrap replace
