...
foreach exp in exp_tsst exp_cpt exp_cpr {
	...
	foreach yvar in time_patient time_indiff time_exponential time_geometric time_auc time_stationarity time_decrimp {

		loc column = 1

		foreach time in 3 4 6 9 {

			reg `yvar' treatment $yfillmiss if time_horizon == `time' & `exp', r cl(sessionnum)
			loc N = `e(N)'
...
loc prehead "\begin{table}[htbp]\centering \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} \caption{Treatment effects on temporal discounting by time horizon} \label{tab:REG-paneltime} \maxsizebox*{\textwidth}{\textheight}{ \begin{threeparttable} \begin{tabular}{l*{`columns'}{c}} \toprule"
...
esttab col* using "$tab_dir/REG-paneltimeall.tex", booktabs cells(none) nonum mtitle("\specialcell{0 mo.\\-- 3 mo.}" "\specialcell{0 mo.\\-- 6 mo.}" "\specialcell{0 mo.\\-- 12 mo.}" "\specialcell{6 mo.\\-- 12 mo.}" "N" "\specialcell{Joint\\\(p\)-value}") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress wrap replace
esttab col* using "$tab_dir/REG-paneltimeall-frag.tex", booktabs cells(none) nonum mtitle("\specialcell{0 mo.\\-- 3 mo.}" "\specialcell{0 mo.\\-- 6 mo.}" "\specialcell{0 mo.\\-- 12 mo.}" "\specialcell{6 mo.\\-- 12 mo.}" "N" "\specialcell{Joint\\\(p\)-value}") stats(`statnames', labels(`varlabels')) compress wrap replace
