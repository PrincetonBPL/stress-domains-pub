* Syntax: managedataend, [PREPOnly] [Prepmanipula] [AUXdo(filename)] [Raw] [Survey] [BCComplete] [Audit] [Backcheck] [DIRdate(integer)]
...
*	- To pause after Manipula programs (Blaise-to-Stata), specify option pause.
...
*	do-file, the data directory is assumed.
...
program managedataend
	version 11
	syntax [, PREPOnly Prepmanipula AUXdo(string) Raw Checkraw Survey BCComplete Audit Backcheck DIRdate(integer -1) pause MAXDate(passthru) /*
		*/ lag(passthru) IGnoresiddups REView SHOWknown]
	if "`prepmanipula'`auxdo'`raw'`checkraw'`survey'`audit'`backcheck'" == "" error 198
	
	if "`preponly'" != "" & "`prepmanipula'" != "" {
	...
	* I add \ to the end of the directory in case the current directory is C:. Stata will not execute cd "c:", but it will execute cd "c:\".
	...
	mdparam list
	disp
	global datadir "C:\Users\mcollins\Dropbox\UMIP\ENDLINE\Data and program\Data"
	quietly cd "$datadir"
	cap log close
	...
	* Create new date directory.
	...
	local manipula `""Data model merge (all)" "Blaise to Stata (all)""'
	
	* Save ado-files.
	foreach ado in "addcheck" "addchecks" "maxdatedir" "mdcheck" "addchange" {
		quietly copy "`ado'.ado" "Archived/`datedir'/`ado'.ado", replace
	}
	foreach ado in "managedataend" "mdparam" "tokencond" "inmat" "delall" "copyall" "fileexists" "filesize" {
	...
	if "`raw'" != "" {
		...
		foreach msu in `manipula' {
			quietly copy "Data models/Manipula/`msu'.msu" "Data/Archived/`datedir'/`msu'.msu", replace
		}
		cd "Data"
		if "`checkday'" != "" {
	...
	if "`checkraw'" != "" {
		...
		if "`checkday'" != "" {
			quietly copy "make_raw.do" "Archived/`datedir'/make_raw.doo", replace
			quietly copy "convert_umip.do" "Archived/`datedir'/convert_umip.do", replace
		}
	}
