*! v0.4.8 by Ryan Knight 15feb2010
	* Added sort by original variable order
* v0.4.7.1 by Ryan Knight 03feb2010
	* added control for cap tostring not producing _rc if it fails
* v0.4.6 by Ryan Knight 12jan2010
	
program define cfout , rclass
	version 10.1
	
	syntax [varlist] using/ , ID(varname) [noPunct noMATch NAme(string) ALTid(varname) Format(string) Upper Lower noString]
	...
	* Run the discrepency pulling separately if altid is specified
	if "`altid'" != "" {
		foreach X in `varlist' {
			...
			if `r(N)'==0 {
				...
				if _rc {
					mata: st_view(i=.,.,("`id'", "`altid'"), "`isdiff'")
					mata: st_sview(s=.,.,( "_cf`X'", "`X'"),"`isdiff'")
					mata: n = J(rows(s),1,"`X'")
					mata: o = (o \ (strofreal(i `format'),s,n))			
				}
				else {
					mata: st_view(r=.,.,("`id'", "`altid'", "_cf`X'", "`X'"),"`isdiff'")
					mata: n = J(rows(r),1,"`X'")
	...
	di as txt "Total Observations Compared: " as res `q'
