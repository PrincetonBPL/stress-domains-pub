...
syntax, filename(string) plcategory(varname) 

local keepthese longitude latitude `plcategory' 
...
syntax namelist(min=1), obs(integer) filename(string)  plcategory(varname) 

local kinds `namelist'
local kindct: list sizeof kinds
local colors red blue green ltblue yellow orange pink purple 
...
return local bigpaddles "http://maps.google.com/mapfiles/kml/shapes/"

end

// write style corresponding to a shape color,
...
syntax,  kmlfile(string) color(string) lon(real) lat(real) 
