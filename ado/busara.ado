...
program busara
	version 11
	syntax [, Clean Invite replies Respondentdata respondentupdate reminder Tribe Ztreedata Payment Paymentcheck Sunny STudydata Session(passthru) Time(passthru) Attendees(passthru) Project(passthru) Sent(passthru) Identifystart(passthru) Identifyend(passthru)]

		cd "C:\Users\mcollins\Dropbox\Busara\data and program\data"
...
* Save busara.ado
quietly copy "C:\ado\personal\busara.ado" "Archived/`datedir'/busara.ado", replace
...
if "`respondentupdate'" != "" {
	...
	cap noisily respondentupdate, `session' `time'
	if _rc {
		cleanup "`curdir'"
		exit 10
	}
	quietly copy "respondentupdate.ado" "Archived/`datedir'/respondentupdate.ado", replace
}
...
if "`paymentcheck'" != "" {
	...
	cap noisily paymentcheck, `session' `time'
	if _rc {
		cleanup "`curdir'"
		exit 10
	}
	quietly copy "paymentcheck.ado" "Archived/`datedir'/paymentcheck.ado", replace
}
